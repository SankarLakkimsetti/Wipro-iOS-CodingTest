...
class CanadaListCell: UITableViewCell {
    ...
    var canadaEachRowData: Rows! {
        didSet {
            titleLabel.text = (canadaEachRowData.title != nil) ? canadaEachRowData.title : AppConstants.canadaDefaultTitle
            descriptionLabel.text = (canadaEachRowData.description != nil) ? canadaEachRowData.description : AppConstants.canadaDefaultDescription
            profileImageView.loadImageUsingCacheWith(urlString: canadaEachRowData.imageHref, placeHolder: AppConstants.placeholderImage)
        }
    }
